...
- En el código que **todavía** llama a `chrome.*` por callback, los errores se chequean con `chrome.runtime.lastError` dentro del callback y no con try/catch (es lo que exige esa API) — referencias vivas: los callbacks de `chrome.tabs`/`chrome.scripting` en `popup.js`, y `plataforma/chrome/mensajeria.ts`, que usa la forma callback justamente porque es la única que expone `lastError`. En el código ya migrado a un puerto la regla es la contraria: los puertos devuelven promesas, así que el fallo se maneja con `.catch()`/try-catch normal (ver en `shared/state.ts` el `respaldar()`, que además deja el `void` explícito porque es fire-and-forget, y `sincronizarConBackground()`, donde el `lastError` de antes es hoy el rechazo del `PuertoMensajeria`).
