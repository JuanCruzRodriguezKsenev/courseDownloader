...
- **Lo que NO es** (corregido 2026-08-02, dicho mal antes en este mismo doc): su primera línea, `if (!navigator.onLine) return false;`, **no** es un bug. `navigator.onLine === false` es confiable (no hay adaptador); lo que no es confiable es el `true` ante una caída de WAN upstream. Como la guarda sólo corta en el caso `false`, es un atajo para ahorrarse un fetch cuando el browser ya sabe que no hay red, y un corte de WAN igual lo detecta el `fetch` de abajo. El daemon hace exactamente lo mismo en `shared/conexion.js:97`. O sea: acá no hay un falso positivo que arreglar — el motivo del cambio es sólo la duplicación.
- **Fix propuesto**: reemplazar el cuerpo por `await Conexion.verificarAhora()` + leer `.internet` del snapshot que devuelve, y borrar el `fetch`, el `AbortController` y el `navigator.onLine` locales. Se elige `verificarAhora()` y no `get()` para **conservar la semántica actual** (sondear fresco justo antes de arrancar) en vez de leer estado de hasta 3s de antigüedad; el cambio es de dueño del sondeo, no de momento. Nota: `verificarAhora()` chequea servidor **e** internet, así que el gate debe seguir mirando sólo `.internet` para no cambiar qué bloquea el arranque.
...
- **Estado**: 🟠 arreglado en la rama `feat/queue-daemon`, **sin mergear**: espera la verificación en navegador (ver el ítem de abajo). `queue.js` v1.3.0 reemplaza el cuerpo por `await Conexion.verificarAhora()` + `.internet`; `queue.test.js` stubea el daemon en vez de `fetch`, afirma que la feature **no** hace `fetch` propio y suma el caso "servidor caído + internet OK ⇒ arranca igual" que fija el gate. Suite 187 → 188, warnings de lint 9 → 8 (se fue el `catch (e)` sin usar del sondeo borrado).
