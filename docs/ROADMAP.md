...
- [~] Extraer `popup/features/queue.js` (cola de descarga, `encolarItemsEnCaliente`, cancelación, reintentos). 🟡 parcial (2026-07-17): en `QueueFeature.crear(ctx)` (con `queue.test.js`, 6 tests) ya viven las mutaciones de la cola (`encolarItemsEnCaliente` + `quitarItemsDeColaEnLote`) y la cancelación de descarga (`solicitarFrenadoSuave` + `abortarRafagaInmediata`). Pendientes de mover acá: `ejecutarReintentoDeCola` (hoy teñido de recuperación de conexión, y ya es dependencia de `serverConnection` vía `onReintentarCola`) y el arranque de descarga (`btnStartQueue` → `iniciar_descarga_cola`, que arrastra `verificarRed`/`congelarUI`/`mostrarAlert`). `popup.js` → v5.8.1.
