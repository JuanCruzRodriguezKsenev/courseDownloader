...
- [~] Extraer `popup/features/queue.js` (cola de descarga, `encolarItemsEnCaliente`, cancelación, reintentos). 🟡 parcial (2026-07-17): extraídas las mutaciones de la cola `encolarItemsEnCaliente` + `quitarItemsDeColaEnLote` (`QueueFeature.crear(ctx)`, con `queue.test.js`, 4 tests). Pendientes de mover acá: `ejecutarReintentoDeCola` (hoy teñido de recuperación de conexión, y ya es dependencia de `serverConnection` vía `onReintentarCola`), el arranque de descarga (`btnStartQueue` → `iniciar_descarga_cola`) y la cancelación suave/dura. `popup.js` → v5.8.0.
