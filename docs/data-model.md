...
| `colaPausadaPorError` | `boolean` | `false` | Si la cola está pausada por un fallo (del portal, de la red o del backend Bun). |
| `tipoDeErrorConexion` | `"internet" \| "servidor" \| "sesion" \| "bloqueo" \| "desconocido" \| ""` | `""` | Por qué se pausó. **Sólo `"internet"`, `"servidor"` y `"desconocido"` entran al autoheal** (`TIPOS_CON_AUTOHEAL` en `core/cola/procesadorCola.ts`) — son los únicos que el daemon `Conexion` puede ver recuperarse. `"sesion"` (no hay sesión iniciada, o el token del portal venció) y `"bloqueo"` (el portal rechaza de forma sistémica: `Referer`, hotlink del CDN) quedan afuera a propósito: la red nunca estuvo caída, así que la alarma reintentaría en loop contra algo que no se arregla solo. `"desconocido"` es el fallback y **reemplazó a `"internet"` el 2026-08-07**: se llega ahí justo cuando el daemon acaba de medir la red OK, así que llamarlo "internet" era mostrarle al usuario una causa falsa. La lista positiva vive en el módulo y **`pausarPorError` y `alDispararAutoheal` tienen que usar la misma**: si divergen, vuelve el loop de 12 s. |
