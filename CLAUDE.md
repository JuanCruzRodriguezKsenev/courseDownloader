...
- **`popup.js`** (+ `popup/scraper.js`, `renderers.js`, `popup/features/*`) — runs in the popup window. Orchestrates all UI state, tab switching (Disponibles/Cola), filtering/search, and drives DOM scraping by injecting `Scraper.escanearAulaVirtual` into the active Ramón Net tab via `chrome.scripting.executeScript`. Talks to the service worker over IPC actions like `iniciar_descarga_cola`, `inyectar_items_en_cola_activa`, `obtener_estados_en_progreso`. Cohesive slices of popup behavior are being extracted into **`popup/features/`** self-contained modules (an ongoing feature-driven split — see `docs/adr/0005-feature-driven-popup-split.md`, `docs/ROADMAP.md` Fase 2): each exports a factory `Feature.crear(ctx)` that receives its dependencies (DOM `nodos`, callbacks into `popup.js`) via a `ctx` object rather than reaching into the popup's closure. Extracted so far: `serverConnection.js` (connection UI + auto-heal recovery); the onboarding tour was extracted as a vanilla feature and has since become a Preact island (below). Add new UI concerns as features in this shape, not as more free functions in `popup.js`. **Preact islands (in-progress, no build):** cohesive UI regions are also being migrated to Preact — vendored build-less as `popup/vendor/htm-preact-standalone.module.js` and loaded via `<script type="module">` (coexists with the vanilla code; the service worker stays vanilla). See `docs/adr/0006-adopt-preact-islands-in-popup.md` and the live status map in `docs/preact-migration.md`. Islands done: (1) `popup/features/conexionHeader.preact.js` owns the header status dot, derived from the `Conexion` daemon (a `useConexion()` hook subscribes to it) — so that dot is no longer painted imperatively anywhere; (1b) `popup/features/rutaDisco.preact.js` owns the disk-path text (`#preact-pc-path`, the `📁 PC:` row) — the first island NOT derived from `Conexion` (the path comes from `BunClient`), so it carries its own store-bridge `window.RutaDisco` (`mostrar`/`cargando`/`get`) that the ~7 former `nodos.pcPath` writers now push to; its boundary is deliberately just the text (the `.path-bar` `<section>`'s Explore button, subject input, and the `.path-bar.offline` class toggle stay vanilla); (2) `popup/features/bannerConexion.preact.js` owns the connection-down banner (`.server-error-card`) — the delicate case, since that banner used to render *inside* `#ui-list` (shared with the class list). It now lives in its own sibling root (`#preact-banner`), and `serverConnection.js` hides `#ui-list` (`display:none` + empties it) while the banner shows, restoring it on reconnect; its store-bridge `window.BannerConexion` (`mostrar(tipo)`/`ocultar()`/`get()`) is pushed by `reaccionarAConexion` (visibility is business logic, not a pure `Conexion` derivation), and the 3 former `querySelector('.server-error-card')` flag reads became `BannerConexion.get()`; (3) `popup/features/onboarding.preact.js` owns the whole welcome-tour overlay (`#preact-onboarding`), with the folder-slide's server status also derived from `Conexion` via `useConexion()` (which removed the imperative `actualizarEstadoServidorOnboarding` sink that `serverConnection.js` used to push). It bridges to the still-vanilla `popup.js` via `window.OnboardingFeature.crear({ btnHelp, onExplore, onComplete })`, keeping the old feature's factory signature. When adding a Preact island, its DOM boundary must be a region the vanilla code does not hold `nodos.*` references to (avoid stale refs).
