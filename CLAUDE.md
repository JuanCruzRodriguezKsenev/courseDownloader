...
- **Run tests**: `npm test` (single run) or `npm run test:watch`. To run one file/test: `npx vitest run core/util/texto.test.ts` or `npx vitest run -t "<test name>"`. **There is no `vitest.config.*` in the repo** — the suite runs on Vitest's defaults, which means the `node` environment: a test that touches the DOM (a `popup/features/` feature, any Preact island) needs `// @vitest-environment jsdom` as its own docblock at the top of the file, or it dies with `document is not defined`. 12 of the 25 files carry one. Modules are ES modules that also publish their object as a global side-effect (`globalThis.X = X; export default X`) — the export feeds the bundler and the tests, the global keeps ~200 existing call-sites working untouched. See `docs/coding-standards.md`. **`docs/testing.md` is the canonical home for the suite's size and coverage narrative — read the count there, don't re-state it here.** The test files, for orientation: `core/util/texto.test.ts`, `core/util/reintentos.test.ts`, `core/estado/appState.test.ts`, `core/conexion/conexion.test.ts`, `core/backend/bunClient.test.ts`, `core/puertos/almacenamientoEnMemoria.test.ts`, `core/puertos/mensajeriaEnMemoria.test.ts`, `core/puertos/programadorEnMemoria.test.ts`, `core/historial/historialFallos.test.ts`, `popup/features/serverConnection.test.js`, `popup/features/queue.test.js`, `popup/features/filters.test.js`, `popup/features/faceta.test.js`, `popup/features/conexionHeader.preact.test.js`, `popup/features/onboarding.preact.test.js`, `popup/features/rutaDisco.preact.test.js`, `popup/features/bannerConexion.preact.test.js`, `popup/features/listaClases.preact.test.js`, `popup/features/campanita.preact.test.js`, `sitio/ramonnet/parserTitulos.test.js`, `sitio/ramonnet/resolverManifiesto.test.js`, `core/hls/hlsEngine.test.ts` (includes the 6-worker pool + the 4xx-retry path), `core/cola/estadoSesion.test.ts`, `core/cola/procesadorCola.test.ts`, `background.test.js`.
- **Lint**: `npm run lint` (ESLint 9 flat config, rules `no-undef`/`no-unused-vars`/`eqeqeq`; `.ts` goes through typescript-eslint). When you add a new cross-file global (a `globalThis.X` consumed elsewhere), add it to `globalesDelProyecto` in `eslint.config.js` or `no-undef` will false-positive. The baseline is **0 errors and 0 warnings** since 2026-08-03 (the 6 that used to be tolerated were unused `catch` bindings and unused args) — a new *warning* is now a regression too, not just an error. `docs/testing.md` stays the canonical home for the number.
- **Typecheck**: `npx tsc --noEmit` (must stay clean). TypeScript is pinned to 5.x on purpose — typescript-eslint does not support TS 7 yet, and losing lint on the migrating core is worse than the compiler's speed gain. **`tsconfig.json`'s `include` list is what it actually covers, and that is not automatic**: `allowJs` is `false` and both entrypoints are `.js`, so `tsc` skips them and with them the whole import graph — a folder of `.ts` that isn't listed is silently unchecked (`shared/` and `plataforma/` were, from Phase 5b until 2026-08-03). The source folders actually covered are `core/`, `plataforma/` and `sitio/` — `shared/` came off the list in Phase 6a, when the folder stopped existing. **Don't read `entrypoints/**/*` in the list as coverage** — it's there, but both entrypoints are `.js`, so they're skipped along with everything they import; that line is exactly the trap, not the escape from it. When a migration puts `.ts` under a new root, add it to `include` in the same change and confirm with `npx tsc --noEmit --listFiles`.
...
- **If `plataforma/composicion.ts` has to import it** → moving it onto a port forces converting it to TypeScript **in the same slice** (this is what happened to `AppState` and the connection daemon in Phase 5b, and it's what made Phase 6a a prerequisite of Phase 6: the engine couldn't land in `core/` while `Utils` was a `.js` global).
...
Two corollaries worth knowing before you cut. **First**: a module that stops publishing its own global and lets `composicion.ts` publish it makes that global appear **later** in the load chain — check nobody consumes it at module-evaluation time, because the bundler won't tell you. That check is what made the `Utils` split of Phase 6a safe (every call site was inside a function). **Second**: the reverse constraint also bites — `composicion.ts` can't import `background.js` either, which is why Phase 6b had to pull the failure notification, the legacy disk dump and the progress mirror out of the service worker before it could assemble the queue.
...
- **`Scraper.escanearAulaVirtual` (`sitio/ramonnet/scraper.js`) runs in the portal's tab, not in the extension** — `popup.js` injects it with `chrome.scripting.executeScript`. It must stay **self-contained and serializable**: it can't reference *any* extension global (`Utils`, `SitioActivo`, `SitioRamonNet`) **nor even a module-level constant of its own file**; anything it needs is passed through `args`. Hoisting a selector out of it into `config.ts` — the move the previous bullet otherwise asks for — silently breaks scanning at runtime, and nothing catches it: not the bundler, not lint, not `tsc`, and not the suite (there is no `scraper.test.js`). Verify in the browser.
